...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        quicksort(inseridos,0,inseridosIndex-1,0);
        sortDesempate(inseridos,inseridosIndex);
       //sortDesempate(inseridos,inseridosIndex);
        z=0;
        while(inseridos[z]!= null&&z<10){
  ...
  //acabei nao usando pra nada mas vou deixar se precisar depois
    ...
    public static void sortDesempate(Personagem[] array, int n) {
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
            for (int j = (i + 1); j < n; j++) {
                if (array[menor].getHouse().equals(array[j].getHouse()) &&
    ...
    //selecao so os 10 primeiros
    public static void sort(Personagem[] array,int n) {
    
        for (int i = 0; i < (10); i++) {
            int menor = i;
            for (int j = (i + 1); j < n; j++) {
                if (array[menor].getBirthDate().after(array[j].getBirthDate())) {
                    menor = j;
                }
            }
            swap(array, menor, i); 
    ...
    public static void intercalar(Personagem[] array, int esq, int meio, int dir) {
        ...
        while (i < n1 && j < n2) {
            int comparison = a1[i].getActorName().compareTo(a2[j].getActorName());
            if (comparison < 0 || (comparison == 0 && a1[i].getName().compareTo(a2[j].getName()) < 0)) {
                array[k++] = a1[i++];
            } else {
                array[k++] = a2[j++];
            }
    ...
    public static void quicksort(Personagem[]array,int esq, int dir,int contador) {
        int i = esq, j = dir;
        int pivo = (dir+esq)/2;
        while (i <= j) {
            while (array[i].getHouse().compareTo(array[pivo].getHouse())<0) i++;
            while (array[j].getHouse().compareTo(array[pivo].getHouse())>0) j--;
            if (i <= j) {
                swap(array,i, j);
                i++;
                j--;
        ...
        if (esq < j&&contador<10)  quicksort(array,esq, j,++contador);
        if (i < dir&&contador<10)  quicksort(array,i, dir,++contador);
    }
